...
@RestController
@RequestMapping("/api/hookah-kafka")
public class HookahKafkaResource {

    private final Logger log = LoggerFactory.getLogger(HookahKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public HookahKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
